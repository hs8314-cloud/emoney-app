...
export default async function KpiInputPage() {
  ...
  const [r1, { data: allEmps }, { data: affiliations }, { data: existingKpi }] = await Promise.all([
    supabase.from('performance_deals').select('id, title, calc_logic, ratio, ratio_changes, employee_id, calc_type, kpi_label_1, kpi_label_2, kpi_1_percent').eq('is_active', true).order('created_at'),
  ...
  // ratio_changes 컬럼 미존재 fallback
  const allDeals: any[] | null = r1.error
    ? (await supabase.from('performance_deals').select('id, title, calc_logic, ratio, employee_id, calc_type, kpi_label_1, kpi_label_2, kpi_1_percent').eq('is_active', true).order('created_at')).data as any[]
    : r1.data as any[]
